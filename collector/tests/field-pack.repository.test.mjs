...
function loadNamedFunction(sourceText, functionName, dependencies = {}) {
  const source = extractFunctionBlock(sourceText, functionName);
...
test("assignment submission draft route rejects revision mismatch before repository upsert", () => {
  const ctx = createTestContext();
  try {
    const item = ctx.createItem("Draft Route Revision Guard");
    const assignee = ctx.createUser("draft-route-guard");
    ctx.repo.createFieldPack({
      content_item_id: item.id,
      status: "ready_for_field",
    });
    ...
    const helper = loadNamedFunction(indexSource, "handleAssignmentSubmissionDraftPut", {
      repo: ctx.repo,
      deriveAssignmentDraftExpiryIso: () => "2099-01-01T00:00:00.000Z",
      resolveAssignmentCurrentRound: (value) => Math.max(1, Number(value?.revision_round || 0) + 1),
      validateAssignmentSubmissionDraftExpectedRevisionRound: (value) => {
        const round = Number(value || 0);
        return Number.isInteger(round) && round > 0 ? round : 0;
      },
      normalizeAssignmentDraftArticlePayload: (payload) => payload,
    });
    let upsertCalled = false;
    const originalUpsert = ctx.repo.upsertAssignmentSubmissionDraft.bind(ctx.repo);
    ctx.repo.upsertAssignmentSubmissionDraft = (...args) => {
      upsertCalled = true;
    ...
    const req = {
      ...
      body: {
        expected_revision_round: Number(assignment.revision_round || 0) + 2,
        article_payload_json: { additional_text: "stale" },
      },
    };

    const result = helper(req, res, assignment, assignee.id);
    ...
    assert.deepEqual(res.body, {
      ...
      expected_revision_round: Number(assignment.revision_round || 0) + 2,
      current_revision_round: Number(assignment.revision_round || 0) + 1,
    });
  } finally {
    ctx.cleanup();
  }
...
test("assignment submission draft route accepts matching revision and upserts draft", () => {
  const ctx = createTestContext();
  try {
    const item = ctx.createItem("Draft Route Revision Match");
    const assignee = ctx.createUser("draft-route-match");
    ctx.repo.createFieldPack({
      content_item_id: item.id,
      status: "ready_for_field",
    });
    ...
    const res = createMockResponse();
    const expectedRound = Number(assignment.revision_round || 0) + 1;
    const req = {
      params: { id: String(assignment.id) },
      body: {
        expected_revision_round: expectedRound,
        article_payload_json: { additional_text: "fresh" },
      },
    };
    ...
    assert.equal(res.body.revision_round, expectedRound);
    assert.equal(result.article_payload_json.additional_text, "fresh");
    assert.equal(result.revision_round, expectedRound);
  } finally {
    ctx.cleanup();
  }
});
