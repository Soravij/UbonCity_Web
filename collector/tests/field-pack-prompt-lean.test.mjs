...
test("lean off: buildPromptInput returns all top-level keys and full evidence_blocks fields", () => {
  ...
  for (const key of expectedTopKeys) {
    assert.ok(key in result, `expected top-level key "${key}" when lean is off`);
  }
  ...
  for (const key of expectedBlockKeys) {
    assert.ok(key in block, `expected evidence_block key "${key}" when lean is off`);
  }
});

test("lean on: buildPromptInput removes specified top-level keys and trims evidence_blocks fields", () => {
  process.env.COLLECTOR_FIELD_PACK_LEAN = "on";
  try {
    const result = buildPromptInput(createItem());
    ...
    for (const key of removedTopKeys) {
      assert.ok(!(key in result), `expected top-level key "${key}" to be removed when lean is on`);
    }
    ...
    for (const key of keptTopKeys) {
      assert.ok(key in result, `expected top-level key "${key}" to be kept when lean is on`);
    }
