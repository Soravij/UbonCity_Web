...
const TEMP_ROOT = path.join(CWD, "tmp-runtime-article-revision-loop-smoke");
...
const ADMIN_AUTH = resolveSmokeActor({
  label: "article revision loop browser smoke admin login",
...
const EDITOR_AUTH = resolveSmokeActor({
  label: "article revision loop browser smoke editor login",
...
const INITIAL_UPDATED_TITLE = "Article Revision Loop Smoke First Draft";
const INITIAL_UPDATED_EXCERPT = "Smoke excerpt for first review submission";
const INITIAL_UPDATED_HEADING = "Smoke heading for first review submission";
const REVISION_UPDATED_TITLE = "Article Revision Loop Smoke Revised Draft";
const REVISION_UPDATED_EXCERPT = "Smoke excerpt after revision loop";
const REVISION_UPDATED_HEADING = "Smoke heading after revision loop";
...
async function createItem(token) {
  const result = await requestJson("/api/items", {
    ...
    token,
    body: {
      title: "Article Revision Loop Browser Smoke",
      type: "place",
...
async function saveEditorWork(token, itemId, coverAssetUrl = "") {
  ...
  const result = await requestJson(`/api/items/${itemId}/editor-work`, {
    ...
    body: {
      ...
      draft: {
        draft_title: "Article Revision Loop Browser Smoke",
...
async function waitForDraftMatch(token, itemId, predicate, description, timeoutMs = 15000) {
  return waitForArticleProcessMatch(
    token,
    itemId,
    (processPayload) => predicate(processPayload?.latest_draft || null, processPayload),
    description,
    timeoutMs,
  );
}
...
async function main() {
  ...
  try {
    ...
    assertLoginRole(adminLogin, ["admin", "owner"], "article revision loop browser smoke admin");
    assertLoginRole(editorLogin, ["editor"], "article revision loop browser smoke editor");
    ...
    try {
      ...
      await evaluate(cdp, 'document.getElementById("btn-assign-editor")?.click(); true;');
      await waitForEditorialAssignment(adminLogin.token, itemId, editorId, 15000);
    } catch {
    ...
    await evaluate(cdp, `(() => {
      ...
      if (title) {
        title.value = ${JSON.stringify(INITIAL_UPDATED_TITLE)};
        title.dispatchEvent(new Event("input", { bubbles: true }));
      }
      const excerpt = document.getElementById("article-excerpt");
      if (excerpt) {
        excerpt.value = ${JSON.stringify(INITIAL_UPDATED_EXCERPT)};
      ...
      if (firstBlock) {
        firstBlock.value = ${JSON.stringify(INITIAL_UPDATED_HEADING)};
    ...
    await waitForItemMatch(
      editorLogin.token,
      itemId,
      (currentItem) => String(currentItem?.title || "").trim() === INITIAL_UPDATED_TITLE && String(currentItem?.summary || "").trim() === INITIAL_UPDATED_EXCERPT,
      "saved first draft workspace changes",
      15000,
    );
    ...
    await waitForArticleProcessStatus(editorLogin.token, itemId, "ready_for_review", 15000);

    await openPageAs(cdp, adminLogin.token, `/article-submit.html?id=${itemId}`);
    await waitForCondition(cdp, `window.location.pathname.endsWith("/article-submit.html") && Boolean(document.getElementById("btn-approve-sync")) && document.getElementById("btn-submit-review") === null`, 15000);
    await waitForCondition(cdp, `!document.getElementById("btn-request-revision")?.disabled`, 10000);

    await evaluate(cdp, `(() => {
      const note = document.getElementById("review-note");
      if (note) {
        note.value = "browser smoke requested revision";
        note.dispatchEvent(new Event("input", { bubbles: true }));
      }
      return true;
    })()`);
    await evaluate(cdp, 'document.getElementById("btn-request-revision")?.click(); true;');
    await waitForArticleProcessStatus(adminLogin.token, itemId, "revision_requested", 15000);
    await waitForCondition(cdp, `window.location.pathname.endsWith("/article-workspace.html")`, 15000);

    await openPageAs(cdp, editorLogin.token, `/article-workspace.html?id=${itemId}`);
    await waitForCondition(cdp, `window.location.pathname.endsWith("/article-workspace.html") && Boolean(document.getElementById("btn-submit-review"))`, 15000);
    await waitForCondition(cdp, `!document.getElementById("btn-submit-review")?.disabled`, 10000);
    await evaluate(cdp, `(() => {
      const title = document.getElementById("article-title");
      if (title) {
        title.value = ${JSON.stringify(REVISION_UPDATED_TITLE)};
        title.dispatchEvent(new Event("input", { bubbles: true }));
      }
      const excerpt = document.getElementById("article-excerpt");
      if (excerpt) {
        excerpt.value = ${JSON.stringify(REVISION_UPDATED_EXCERPT)};
        excerpt.dispatchEvent(new Event("input", { bubbles: true }));
      }
      const firstBlock = document.querySelector('#article-blocks textarea[data-block-field="text"]');
      if (firstBlock) {
        firstBlock.value = ${JSON.stringify(REVISION_UPDATED_HEADING)};
        firstBlock.dispatchEvent(new Event("input", { bubbles: true }));
      }
      const note = document.getElementById("review-note");
      if (note) {
        note.value = "browser smoke revised and resubmitted";
        note.dispatchEvent(new Event("input", { bubbles: true }));
      }
      return true;
    })()`);
    await evaluate(cdp, 'document.getElementById("btn-save-workspace")?.click(); true;');
    await waitForDraftMatch(
      editorLogin.token,
      itemId,
      (latestDraft) => String(latestDraft?.draft_title || "").trim() === REVISION_UPDATED_TITLE && String(latestDraft?.excerpt || "").trim() === REVISION_UPDATED_EXCERPT,
      "saved revised workspace changes",
      15000,
    );
    await waitForCondition(cdp, `!document.getElementById("btn-submit-review")?.disabled`, 10000);
    await evaluate(cdp, 'document.getElementById("btn-submit-review")?.click(); true;');
    ...
    try {
      finalProcess = await waitForArticleProcessStatus(adminLogin.token, itemId, "ready_for_review", 15000);
    } catch (err) {
      const submitDiagnostics = await evaluate(cdp, `(() => ({
      ...
      throw new Error(`${String(err?.message || err)}; submit diagnostics: ${JSON.stringify(submitDiagnostics)}`);
    }
    assert(String(finalProcess?.status || "").trim().toLowerCase() === "ready_for_review", `final article-process status should be ready_for_review after resubmit: ${JSON.stringify(finalProcess)}`);
    assert(String(finalProcess?.workflow_model?.production_state || "").trim().toLowerCase() === "in_review", `final production_state should be in_review after resubmit: ${JSON.stringify(finalProcess)}`);
    assert(String(finalProcess?.active_editorial_assignment?.state || "").trim().toLowerCase() === "resubmitted", `final assignment state should be resubmitted after resubmit: ${JSON.stringify(finalProcess)}`);
    ...
    console.log(JSON.stringify({
      ...
      checks: {
        ...
        workspace_submit_review_initial: true,
        review_request_revision: true,
        workspace_loaded_for_revision: true,
        workspace_submit_review_after_revision: true,
      },
    }, null, 2));
  } catch (err) {
    keepArtifacts = true;
    console.error(`smoke-article-revision-loop-browser: FAILED - ${String(err?.message || err)}`);
    ...
    try {
      ...
      keepArtifacts = true;
      console.error(`smoke-article-revision-loop-browser: backend stop issue - ${String(stopErr?.message || stopErr)}`);
      process.exitCode = 1;
    }
    if (!keepArtifacts) {
      await rmTempRoot();
