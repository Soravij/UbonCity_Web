...
- Must not become cover/gallery/inline usable media automatically.
...
- Must come from selected local collector `content_assets`.
- Eligible assets must be collector-controlled local/nas assets: `storage_disk` in `local` or `nas`, non-http(s) `storage_path`, image mime type when available.
- External/reference media cannot be used as publish media unless manually rights-verified, imported into collector-controlled local storage, and selected as a local asset.

## Stage-specific Blocker Policy

### Raw Data / Clean Prep / Field Pack Draft

- Must not hard-block only because no local usable article media is selected.
- May proceed with reference/evidence media.
- If no local usable asset exists, show warning or `missing_local_asset` / `missing_cover` state.
- AI may use reference/evidence media for internal analysis, but output must not mark those media as usable/publish assets.

### Article Workspace

- May open with missing-cover / needs-local-asset state.
- Must not auto-promote external/reference media into usable media.

### Submit Admin Review / Final Handoff / Publish Export

- Must remain hard-blocked unless selected local usable cover/assets exist.
- External/reference media cannot satisfy this blocker.

## Error Placement

The error `"usable article media must be selected from uploaded local assets"` is correct for:
- Submit Admin Review
- Final Handoff
- Publish / Export
...
## Existing Safety Rules (unchanged)
...
### Stage-specific Media Readiness Policy

See Admin Review Media Policy above for full stage-specific blocker rules. Summary:

| Stage | References OK? | Usable local media required? | Hard block if missing? |
|---|---|---|---|
| Raw Data / Clean Prep | Yes (reference-only) | No | No |
| Field Pack Draft | Yes (reference-only) | No | No |
| Article Workspace | No | Encouraged | No (missing-cover state) |
| Submit Admin Review | No | Yes | Yes |
| Final Handoff / Publish | No | Yes | Yes |
