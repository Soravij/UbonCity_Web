...
import { COLLECTOR_HEAD_PARTIAL, COLLECTOR_BRAND_PARTIAL, COLLECTOR_HEAD_CSS_PARTIAL, COLLECTOR_AUTH_PARTIAL } from "./header-partials.mjs";
...
function rewriteCollectorHtmlAssetUrls(htmlText, htmlFilePath) {
  let html = htmlText
    ...
    .replaceAll("<!--CH_HEAD_CSS-->", COLLECTOR_HEAD_CSS_PARTIAL)
    .replaceAll("<!--CH_AUTH-->", COLLECTOR_AUTH_PARTIAL);
  return html.replace(
    /((?:src|href)=["'])([^"']+)(["'])/gi,
  ...
  try {
    ...
    const status = /current field pack not found/i.test(msg)
      ...
      : /notes\/reason is required|content_item_id is required/i.test(msg)
        ? 400
        : err?.code === "OPEN_ASSIGNMENT_BLOCKS_RETURN_TO_CLEAN" || /publish-ready|published state/i.test(msg)
          ? 409
          : 400;
    res.status(status).json({ error: msg });
  }
