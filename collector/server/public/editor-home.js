...
function wire() {
  ...
  qs("btn-open-article-workspace")?.addEventListener("click", () => {
    window.location.assign(workspaceUrl("/article-workspace.html", landingItemId));
  });
  qs("btn-open-event-workspace")?.addEventListener("click", () => {
  ...
  qs("btn-login")?.addEventListener("click", async () => {
    try {
      ...
      applyAuthUI();
      setAuthStatus(`เข้าสู่ระบบเป็น ${state.user?.display_name || state.user?.email || "-"} (editor)`);
    } catch (error) {
