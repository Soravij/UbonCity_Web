...
- 2026-08-14: `current_field_pack_id` (`content_workflow_models`) is not kept in sync with `field_packs.is_current=1` by any write path — `syncWorkflowHeadPointers` (`collector/db/repository.mjs:5039`) is defined but never called; current DB shows 0/12 items drifted, but this is a structural risk rather than a guarantee.
- 2026-08-14: `PUT /api/items/:id/editor-work` (`collector/server/index.mjs:8880`): a `field_pack.id` that is nonexistent or belongs to another content item, sent together with `status=ready_for_field` while `production_state` is still pre-generated, surfaces as `409 FIELD_PACK_HEAD_NOT_GENERATED` instead of the more accurate ownership error `saveItemWithFieldPack()` would otherwise throw (`collector/db/repository.mjs:10132-10139`) — the request is still correctly rejected, only the error message is wrong.
- 2026-08-14: `collector/server/public/item-editor.js`: `wire()` (`:6105`) enables `btn-save` before `loadCurrentFieldPack()` (`:6119`) resolves, so an early click can submit `editor-work` with `field_pack.id` still unset; currently harmless because `fp-status` also still holds its HTML default `draft` (`item-editor.html:336`) at that point, but it's a race affecting the whole editor-work form, not just the field pack. Update 2026-08-15 (`fix/editor-action-guards-stale-after-fieldpack-load`, merged `a043578`): the *post-load staleness* half of this pattern — `applyEditorActionGuards()` reading `fp-status` before `loadCurrentFieldPack()`/`fillFieldPackForm()` had written it, leaving `btn-next-export` stuck disabled forever — is fixed (guards now re-apply after the form is actually filled). The remainder described here, the *pre-load early-click* window where `btn-save` is clickable before the field pack has loaded at all, is a different failure mode and is still open.
- 2026-08-15: `renderStepFourNextPanel()` (`item-editor.js:4696-4780`) and `applyEditorActionGuards()` (`item-editor.js:234-243`) each independently compute the same `getEditPermissionGuard()` / `getEditorAssignmentGuard()` result to decide, respectively, whether the `#btn-step4-next-panel` proxy button and the real `#btn-next-export` button should be enabled. Every current call site invokes both together, so the two never disagree today, but this is an undocumented invariant, not an enforced one — a future call site that refreshes one without the other would silently desync the proxy from its target again (the exact silent-click bug fixed in `fix/editor-action-guards-stale-after-fieldpack-load`).
...
- 2026-08-15 (`feat/article-intake-backward-widget`, merged `658dbc4`): the article-intake backward-transition widget (`writing_assigned` -> `field_review`) has not been verified on Runtime for any of the 3 required cases — owner actually clicking the transition through, a non-`writing_assigned` item type confirmed not to show the button, and the no-redirect case where the queue must be patched client-side — because the dev DB has no item sitting in `writing_assigned` and the browser cannot reach `localhost`. Verify on Runtime before treating this widget as production-confirmed.
- 2026-08-15: the widget's hook point on `loadSelectedItem()` is only safe because of the current call ordering — `article-intake.js:802` and `:880` set `state.itemId` directly without going through `loadSelectedItem()`. If a future change routes either of those call sites through `loadSelectedItem()`, re-check the widget's hook assumption.
- 2026-08-15: the `state.items` patch in `article-intake.js` is load-bearing, not incidental cleanup debt — do not remove it. When `surface=article_intake`, the page does not redirect after a backward transition, so without the patch the queue would keep showing the stale pre-transition item state.
- 2026-08-15: `close_assignment` (`collector/server/index.mjs:2833`, `PATCH /api/assignments/:id/state`) has no UI caller in any state. An item with a field round stuck at `assigned` currently has no way to be released through the website.
- 2026-08-15: `field_pack_assignments` was previously suspected to be a bug — re-checked during the article-intake backward-widget audit and confirmed it is not: it is an optional supplementary table with no effect on any display or guard logic. Treat as closed; no prior PROJECT_STATE.md line existed to correct.
