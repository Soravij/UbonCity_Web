...
- 2026-07-23: MySQL migrations are not a fresh/rerunnable schema source: 001 uses `ADD COLUMN IF NOT EXISTS`, unsupported by Runtime MySQL 8.0.46; 006/007/011/015/016 are not rerunnable. A migration runner + ledger now exists (`backend/scripts/migrate.js`: `schema_migrations` :14-22, `classify()` :42-49, `cmdMigrate` GET_LOCK + checksum :138-192) with npm scripts `migrate` / `migrate:init` / `migrate:status` in `backend/package.json`, runner executes only files classified `runner` (n >= 27); 027_ai_usage_log_user_created_idx.sql is the first such file, applied on Runtime 1 Sep 2026, recorded in schema_migrations with source='runner'. The ledger must be seeded before the runner can work: `npm run migrate:init` records 000-026 (baseline/manual). Without it the runner aborts on the FIRST non-runner file it finds that is absent from the ledger — this hard abort is intentional, it guarantees the DB really came through baseline, and is not to be softened into a skip. Runtime bootstrap remains the schema source of truth pending a separate post-Phase-5 migration-system repair. Migrations 025 (`025_align_category_translations.sql`) and 026 (`026_users_role_default.sql` — `users.role` DEFAULT admin→user) are both hardcoded as manual in `classify()`. The obsolete `purgeContentService.js` consumer of `lifecycle_content_map` was removed in e7f06d4, so the table can be physically dropped in Step C.
...
- 2026-07-23: After Step B removes lifecycle, `reviewIngestService.js:81` / `:770` `mirrorImageToBackendStorage()` will still have two production callers: direct-JSON `/review-content/ingest` and `/review-content/event-queue/enqueue`. The final-review smoke covers only multipart `submit-admin-review`, not either fallback path. Decide separately whether to add a gate or retire the fallback.
...
- 2026-08-14: `current_field_pack_id` (`content_workflow_models`) is not kept in sync with `field_packs.is_current=1` by any write path — `syncWorkflowHeadPointers` (`collector/db/repository.mjs:5096`) is defined but never called; current DB shows 0/12 items drifted, but this is a structural risk rather than a guarantee.
- 2026-08-14: `PUT /api/items/:id/editor-work` (`collector/server/index.mjs:8879`): a `field_pack.id` that is nonexistent or belongs to another content item, sent together with `status=ready_for_field` while `production_state` is still pre-generated, surfaces as `409 FIELD_PACK_HEAD_NOT_GENERATED` instead of the more accurate ownership error `saveItemWithFieldPack()` would otherwise throw (`collector/db/repository.mjs:saveItemWithFieldPack :10244`, ownership throw `:10266-10268`) — the request is still correctly rejected, only the error message is wrong. Error code set at `index.mjs:12768`.
- 2026-08-14: `collector/server/public/item-editor.js`: `wire()` (`:5608`) enables `btn-save` before `loadCurrentFieldPack()` resolves, so an early click can submit `editor-work` with `field_pack.id` still unset; currently harmless because `fp-status` also still holds its HTML default `draft` (`item-editor.html:336`) at that point, but it's a race affecting the whole editor-work form, not just the field pack. Init calls `wire()` at `:6117`, then `applyEditorActionGuards()` at `:6128`, then `loadCurrentFieldPack()` at `:6131`. Update 2026-08-15 (`fix/editor-action-guards-stale-after-fieldpack-load`, merged `a043578`): the *post-load staleness* half of this pattern — `applyEditorActionGuards()` reading `fp-status` before `loadCurrentFieldPack()`/`fillFieldPackForm()` had written it, leaving `btn-next-export` stuck disabled forever — is fixed (guards now re-apply after the form is actually filled). The remainder described here, the *pre-load early-click* window where `btn-save` is clickable before the field pack has loaded at all, is a different failure mode and is still open.
- 2026-08-15: `renderStepFourNextPanel()` (`item-editor.js:4698-4777`) and `applyEditorActionGuards()` (`item-editor.js:234-243`) each independently compute the same `getEditPermissionGuard()` / `getEditorAssignmentGuard()` result to decide, respectively, whether the `#btn-step4-next-panel` proxy button and the real `#btn-next-export` button should be enabled. Every current call site invokes both together, so the two never disagree today, but this is an undocumented invariant, not an enforced one — a future call site that refreshes one without the other would silently desync the proxy from its target again (the exact silent-click bug fixed in `fix/editor-action-guards-stale-after-fieldpack-load`).
- 2026-08-15: `saveCurrentFieldPack()` (`item-editor.js:4398-4409`) has no remaining caller in `item-editor.js`; confirmed dead code. Left in place pending a decision on whether to wire it up or remove it.
...
- `GET /analytics/ai-usage` accepts `from` / `to` (absolute `YYYY-MM-DD` dates); date boundaries are computed with string math (`nextDay`) so the inclusive range is timezone-independent; omitting both returns all-time totals
...
- `npm run gate` ใช้ได้แล้วบน Runtime — `scripts/testAll.mjs:55-70` parse TAP summary เขียน `.gate-summary.json`, `scripts/gate.mjs:16-27` อ่านจากไฟล์นั้น
...
- Full role matrix
- Full publish workflow state machine (production state `ready_for_writer` added in c75ca90, but state machine ทั้งชุดยังไม่ครบ)
- Translation policy
