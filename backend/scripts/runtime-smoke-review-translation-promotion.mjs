...
try {
  ...
  await approve(rc); a = await rows("SELECT lang,status FROM review_content_translations WHERE review_content_id=?", [rc]); must(a.every(x => x.status === "published"), "V1 published staging"); const content = (await rows("SELECT public_entity_id FROM review_contents WHERE id=?", [rc]))[0]; let pub = await rows("SELECT lang,description FROM place_translations WHERE place_id=? ORDER BY lang", [content.public_entity_id]); must(pub.map(x=>x.lang).join(",") === "en,lo,th" && pub.every(x=>String(x.description).includes("/uploads/")), "V1 public inline"); report.push([3,"pass"]);
  const v2 = await submit("V2", ["en"], { reuseEditorial: true }); const b = (await rows("SELECT current_batch_uid FROM review_contents WHERE id=?", [rc]))[0].current_batch_uid; a = await rows("SELECT batch_uid,lang,status FROM review_content_translations WHERE review_content_id=? ORDER BY batch_uid,lang", [rc]); must(a.filter(x=>x.batch_uid===batchA).length===2 && a.filter(x=>x.batch_uid===batchA).every(x=>x.status==="published") && a.some(x=>x.batch_uid===b&&x.lang==="en"&&x.status==="review_ready"), "V2 batches"); report.push([4,"pass"]);
  const v2Before = { public: await rows("SELECT lang,LEFT(description,80) AS description FROM place_translations WHERE place_id=? ORDER BY lang", [content.public_entity_id]), public_timestamp_columns: "not present in Runtime place_translations schema", staging: await rows("SELECT batch_uid,lang,status FROM review_content_translations WHERE review_content_id=? ORDER BY batch_uid,lang", [rc]), review: (await rows("SELECT current_batch_uid,lang FROM review_contents WHERE id=?", [rc]))[0] };
  ...
  await approve(rc); const v2After = { public: await rows("SELECT lang,LEFT(description,80) AS description FROM place_translations WHERE place_id=? ORDER BY lang", [content.public_entity_id]), public_timestamp_columns: "not present in Runtime place_translations schema", staging: await rows("SELECT batch_uid,lang,status FROM review_content_translations WHERE review_content_id=? ORDER BY batch_uid,lang", [rc]), review: (await rows("SELECT current_batch_uid,lang FROM review_contents WHERE id=?", [rc]))[0] }; console.error(JSON.stringify({ v2_approve_diagnostics: { before: v2Before, computed: { historical, current, sourceLang, wouldDelete }, after: v2After } }, null, 2)); pub = v2After.public; must(pub.map(x=>x.lang).join(",") === "en,th", "V2 removes lo"); report.push([5,"pass"]);
  await pool.query("INSERT INTO place_translations(place_id,lang,title,description,meta_title,meta_description) VALUES(?,?,?,?,?,?)", [content.public_entity_id,"fr","lifecycle fr","lifecycle","fr","fr"]); const v3 = await submit("V3", ["en"]); await approve(rc); pub = await rows("SELECT lang FROM place_translations WHERE place_id=? ORDER BY lang", [content.public_entity_id]); must(pub.map(x=>x.lang).join(",") === "en,fr,th", "fr preserved"); report.push([6,"pass"]);
  const v4 = await submit("V4", ["en"]); const batchD = (await rows("SELECT current_batch_uid FROM review_contents WHERE id=?", [rc]))[0].current_batch_uid; const r = await fetch(`${backendBase}/review-content/${rc}/reject`, { method:"POST", headers:{authorization:`Bearer ${jwt.sign({id:1,email:"runtime-harness@local.test",role:"owner"},secret,{issuer:process.env.JWT_ISSUER||"uboncity-backend",audience:process.env.JWT_AUDIENCE_BACKEND||"uboncity-backend",expiresIn:"10m"})}`,"content-type":"application/json"},body:JSON.stringify({review_note:"runtime harness reject"})}); must(r.ok,"reject"); a=await rows("SELECT status FROM review_content_translations WHERE review_content_id=? AND batch_uid=?",[rc,batchD]); must(a.every(x=>x.status==="deleted"),"V4 deleted"); pub=await rows("SELECT lang FROM place_translations WHERE place_id=? ORDER BY lang",[content.public_entity_id]); must(pub.map(x=>x.lang).join(",") === "en,fr,th","public unchanged"); report.push([7,"pass"]);
