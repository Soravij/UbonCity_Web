...
try {
  ...
  await pool.query("INSERT INTO place_translations(place_id,lang,title,description,meta_title,meta_description) VALUES(?,?,?,?,?,?)", [publicEntityId,"fr","lifecycle fr","lifecycle","fr","fr"]); const v3 = await submit("V3", ["en"]); await approve(rc); const v3Identity=(await rows("SELECT public_entity_id FROM review_contents WHERE id=?",[rc]))[0]; must(Number(v3Identity.public_entity_id)===publicEntityId,"V3 preserves public identity"); pub = await rows("SELECT lang FROM place_translations WHERE place_id=? ORDER BY lang", [publicEntityId]); must(pub.map(x=>x.lang).join(",") === "en,fr,th", "fr preserved"); report.push([6,"pass"]);
  const v4 = await submit("V4", ["en"]); const batchD = (await rows("SELECT current_batch_uid FROM review_contents WHERE id=?", [rc]))[0].current_batch_uid; const v4BeforeIdentity=(await rows("SELECT public_entity_id FROM review_contents WHERE id=?",[rc]))[0]; const r = await fetch(`${backendBase}/review-content/${rc}/reject`, { method:"POST", headers:{authorization:`Bearer ${jwt.sign({id:1,email:"runtime-harness@local.test",role:"owner"},secret,{issuer:process.env.JWT_ISSUER||"uboncity-backend",audience:process.env.JWT_AUDIENCE_BACKEND||"uboncity-backend",expiresIn:"10m"})}`,"content-type":"application/json"},body:JSON.stringify({review_note:"runtime harness reject"})}); must(r.ok,"reject"); a=await rows("SELECT status FROM review_content_translations WHERE review_content_id=? AND batch_uid=?",[rc,batchD]); must(a.every(x=>x.status==="deleted"),"V4 deleted"); const v4AfterIdentity=(await rows("SELECT public_entity_id FROM review_contents WHERE id=?",[rc]))[0]; must(Number(v4BeforeIdentity.public_entity_id)===publicEntityId&&Number(v4AfterIdentity.public_entity_id)===publicEntityId,"V4 preserves public identity"); pub=await rows("SELECT lang FROM place_translations WHERE place_id=? ORDER BY lang",[publicEntityId]); must(pub.map(x=>x.lang).join(",") === "en,fr,th","public unchanged"); report.push([7,"pass"]);
  console.log(JSON.stringify({ ok:true, item_id:itemId, review_content_id:rc, public_entity_ids:{v1:publicEntityId,v2_reingest:v2ReingestReview.public_entity_id,v2_approve:v2After.review.public_entity_id,v3:v3Identity.public_entity_id,v4_before_reject:v4BeforeIdentity.public_entity_id,v4_after_reject:v4AfterIdentity.public_entity_id}, checks:report }, null, 2));
