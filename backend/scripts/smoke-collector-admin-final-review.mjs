...
function prepareFixture(fixture, version) {
  withRepo((repo, db) => {
    ...
    if (fixture.type === "event") repo.updateItemWithWorkflowHead({ ...item, event_period_text: content.event_period_text, location_text: content.location_text }, "final-review-smoke@local.test");
...
// Event public SELECT has no slug (eventController.js:365); place public SELECT does (placeController.js:764).
async function publicEntry(fixture, items, publicEntityId) { return fixture.type === "event" ? items.find((item) => Number(item.id) === Number(publicEntityId)) : items.find((item) => item.slug === fixture.slug); }
async function assertPublicTranslation(fixture, lang, expectedTitle, publicEntityId) { const entry = await publicEntry(fixture, await publicList(fixture, lang), publicEntityId); assertOk(entry, `${fixture.key}: public ${lang} entry missing`); if (fixture.type === "place") assert.equal(entry.title, expectedTitle, `${fixture.key}: public ${lang} title`); }
...
export async function runCollectorAdminFinalReviewSmoke() {
  ...
  const fixtures = ["approve", "reject", "eventApprove", "eventReject"].map((key, index) => {
    ...
    // Event fixtures intentionally use editorial readiness; field-flow diagnostics are non-blocking while editorial is ready (collector/server/index.mjs:13763).
    return { key, type, itemId: 0, slug: `final-review-${label}`, coverV1: `${label}-v1.png`, coverV2: `${label}-v2.png`, v1: { title: `${label} V1`, body: `<p>${label} body V1</p>`, excerpt: `${label} excerpt V1`, meta_title: `${label} meta V1`, meta_description: `${label} description V1`, translation_langs: ["en", "lo"], ...(type === "event" ? { event_period_text: `${label} period V1`, location_text: `${label} location V1` } : {}) }, v2: { title: `${label} V2`, body: `<p>${label} body V2</p>`, excerpt: `${label} excerpt V2`, meta_title: `${label} meta V2`, meta_description: `${label} description V2`, translation_langs: ["en"], ...(type === "event" ? { event_period_text: `${label} period V2`, location_text: `${label} location V2` } : {}) } };
  });
  ...
  try {
    ...
    const preApprovalPublicEntityIds = Object.fromEntries(await Promise.all(fixtures.map(async (fixture) => [fixture.key, Number((await detail(ids[fixture.key])).public_entity_id || 0)])));
    for (const fixture of fixtures) { const items = await publicList(fixture); const publicId = preApprovalPublicEntityIds[fixture.key]; assertOk(!publicId || !(await publicEntry(fixture, items, publicId)), `${fixture.key}: public before approve`); }
    ...
    for (const fixture of [approve, eventApprove]) { const item = await detail(ids[fixture.key]); v1PublicEntityIds[fixture.key] = Number(item.public_entity_id || 0); assertOk(v1PublicEntityIds[fixture.key] > 0, `${fixture.key}: V1 public entity id missing`); const items = await publicList(fixture); assertOk(await publicEntry(fixture, items, v1PublicEntityIds[fixture.key]), `${fixture.key}: public after approve`); await assertPublicTranslation(fixture, "th", fixture.v1.title, v1PublicEntityIds[fixture.key]); await assertPublicTranslation(fixture, "en", `${fixture.v1.title} EN`, v1PublicEntityIds[fixture.key]); await assertPublicTranslation(fixture, "lo", `${fixture.v1.title} LO`, v1PublicEntityIds[fixture.key]); }
    ...
    for (const fixture of [approve, eventApprove]) { await decide(ids[fixture.key], "approve"); const item = await detail(ids[fixture.key]); assert.equal(Number(item.public_entity_id || 0), v1PublicEntityIds[fixture.key], `${fixture.key}: V2 public entity identity`); await assertPublicTranslation(fixture, "th", fixture.v2.title, v1PublicEntityIds[fixture.key]); await assertPublicTranslation(fixture, "en", `${fixture.v2.title} EN`, v1PublicEntityIds[fixture.key]); await assertPublicTranslation(fixture, "lo", fixture.v2.title, v1PublicEntityIds[fixture.key]); }
