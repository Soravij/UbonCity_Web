...
async function cleanupTestShortcuts() {
  await pool.query("DELETE FROM shortcut_places WHERE shortcut_id IN (SELECT id FROM shortcuts WHERE slug LIKE ?)", [`${TEST_FILL_PREFIX}%`]);
  await pool.query("DELETE FROM shortcuts WHERE slug LIKE ?", [`${TEST_FILL_PREFIX}%`]);
  ...
  await pool.query("DELETE FROM places WHERE slug LIKE ?", [`${TEST_SLUG}-%`]);
}
...
test("shortcuts API", async (t) => {
  ...
  await t.test("C10: situation exists with no places returns empty items", async () => {
    ...
    try {
      const res = await fetch(`http://localhost:${port}/api/situations/day-trip/places`);
      const body = await res.json();
      assert.equal(res.status, 200, `expected 200, got ${res.status}`);
  ...
  await t.test("C11: situation places ordered by sort_order ASC", async () => {
    const [[sitRow]] = await pool.query("SELECT id FROM situations WHERE slug = ? LIMIT 1", ["day-trip"]);
    assert.ok(sitRow, "seed situation 'day-trip' must exist");
    const sitId = Number(sitRow.id);
    ...
    try {
      ...
      const testItems = items.filter((it) => placeIds.includes(Number(it.id)));
      assert.equal(testItems.length, 3);
      assert.equal(testItems[0].sort_order, 1);
      assert.equal(testItems[1].sort_order, 2);
      assert.equal(testItems[2].sort_order, 3);
    } finally {
      server.close();
      await pool.query("DELETE FROM situation_places WHERE situation_id = ?", [sitId]);
      for (const pid of placeIds) {
        await pool.query("DELETE FROM places WHERE id = ?", [pid]);
      }
    }
  });
