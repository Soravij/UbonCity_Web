    ...
    reviewAssets: [
      {
        ...
        resolved_source_url: "https://example.com/source-cover.jpg",
...
test("approveReviewContent updates published place image fields from storage_path-backed media", async () => {
  ...
  try {
    ...
    assert.equal(harness.state.mediaAssets[0].storage_path, "uploads/published/places/place-99/cover.jpg");
    assert.equal(harness.state.mediaAssets[0].file_name, "cover.jpg");

    assert.equal(harness.state.places.length, 1);
    assert.equal(harness.state.places[0].image, "https://api-test.uboncity.com/uploads/published/places/place-99/cover.jpg");
    assert.equal(harness.state.places[0].decision_cover_image, "https://api-test.uboncity.com/uploads/published/places/place-99/cover.jpg");
    assert.equal(harness.state.places[0].decision_thumbnail_image, "https://api-test.uboncity.com/uploads/published/places/place-99/cover.jpg");
    assert.notEqual(harness.state.places[0].image, "https://api-test.uboncity.com/uploads/cover.jpg");
