...
function normalizeDecisionAndMedia(row, media = { cover: null, gallery: [], galleryItems: [], inline: [] }) {
  ...
  return {
    ...
    line_url: String(row?.line_url || "").trim() || null,
    facebook_url: String(row?.facebook_url || "").trim() || null,
    website_url: String(row?.website_url || "").trim() || null,
    primary_cta: String(row?.primary_cta || "").trim().toLowerCase() || null,
    tracking_entity_type: String(row?.tracking_entity_type || "").trim().toLowerCase() || null,
