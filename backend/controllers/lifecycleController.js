...
function assertLifecycleInfrastructureReady() {
...
export const getCollectorImportReviewQueue = async (req, res) => {
  try {
    ...
    const items = await listCollectorImportReviews({
      reviewStatus: status,
      sourceSystem,
      sourceContentType,
      search,
    ...
    const statusCounts = await countCollectorImportReviewsByStatus({
      sourceSystem,
      sourceContentType,
    });
    const statusTypeCounts = await countCollectorImportReviewsByStatusAndType({
      sourceSystem,
      sourceContentType,
    });
...
export const rejectCollectorImportReview = async (req, res) => {
  try {
    ...
    const affectedRows = await rejectCollectorImportReviewById({
      reviewId: id,
    ...
    return res.json({
      message: "Rejected",
      item: await getCollectorImportReviewById(id),
    });
  } catch (err) {
    if (String(err?.message || "").includes("Lifecycle infrastructure is not initialized")) {
      return res.status(503).json({ error: "Lifecycle infrastructure is not initialized" });
...
export const getDeletedContentHistory = async (req, res) => {
  try {
    ...
    const offset = Number(req.query?.offset || 0);
    const items = await listContentPurgeAudit({ limit, offset });
    return res.json({
      items: items.map((row) => ({
